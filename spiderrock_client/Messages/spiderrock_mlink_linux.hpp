        ...
        namespace details {
            ...
            class WebSocket : public ISocket {
                ...
                WebSocket() {
                    ...
                    ws->binary(true);
                    //auto lSize = ws->write_buffer_bytes();
                    //std::cout << "Buffer of websocket: " << lSize << std::endl;
                    ws->write_buffer_bytes(1024 * 1024 * 1); //set the buffer to 1MB
                }
                ...
                bool connect(const std::string & host, const std::string & port, const std::string & path, const std::string & query, const std::string & apikey)
                {
                    ...
                    catch (const boost::system::system_error &err)
                    {
                        ...
                        std::cerr << "Error: " << err.what() << " at line " << __LINE__ << std::endl;
                        std::cerr << "Error code: " << last_error << std::endl;
                        std::cerr << "Boost msg: " << boost::system::error_code(last_error, boost::system::generic_category()).message() << std::endl;
                        return false;
                    }
                }
